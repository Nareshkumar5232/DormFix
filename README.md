🏠 DormFix – Fixing Hostel Life, One Issue at a Time

A smart hostel management system where students report issues instantly, admins resolve them efficiently, and every action is tracked transparently.

🎮 Demo (Prototype): 
📊 Admin Panel Preview: 

⚠️ Note: DormFix is optimized for web & mobile-first usage to ensure quick issue reporting even on low bandwidth.

🌟 Features

✅ One-click student issue reporting (water, electricity, cleanliness, safety, Wi-Fi, etc.)

✅ Real-time admin dashboard for issue tracking & resolution

✅ Priority-based complaint handling (urgent / normal / low)

✅ Status updates: Reported → In Progress → Resolved

✅ Notification system for students & wardens

✅ Issue history & analytics for better hostel management

🛠 How It Works

Student selects an issue category and submits a complaint with optional photo/video.
DormFix assigns the issue to the relevant admin/maintenance staff.
Admin updates progress and resolution status in real time.
Student receives notifications until the issue is resolved.
All issues are logged for accountability and analysis.

📂 Repo Structure
DormFix/<br>
│
├── Student_App/ <br>
│   ├── issue_report/<br>
│   ├── uploads/<br>
│   └── notifications/<br>
│
├── Admin_Dashboard/<br>    
│   ├── issue_tracker/<br>
│   ├── analytics/<br>
│   └── staff_assignment/<br>
│
├── Backend/<br>             
│   ├── auth/<br>
│   ├── issues/<br>
│   └── users/<br>
│
├── Database/<br>            
│
├── Docs/<br>                
│
├── LICENSE<br>              
└── README.md<br>             

🔔 Issue Categories Supported

🚰 Water & Plumbing
💡 Electricity
🧹 Cleanliness & Hygiene
📶 Internet / Wi-Fi
🔒 Safety & Security
🛏 Room & Furniture
🍽 Mess / Food Complaints
📦 Other / Custom Issues

🚀 Setup for Developers
git clone https://github.com/Nareshkumar5232/DormFix.git


You can modify:

Student_App/ → UI for reporting issues

Admin_Dashboard/ → Issue management & analytics

Backend/ → Authentication, notifications, workflows

🔗 Demo Links

🖥 Student Portal: 
📊 Admin Dashboard: 

🛠 Built With

React / Next.js – Frontend UI
Node.js + Express – Backend API
MongoDB / PostgreSQL – Issue storage
Firebase / Web Push – Notifications
Cloud Storage – Image & media uploads

📈 Future Scope

AI-based issue prioritization
Auto-routing issues to relevant staff
Mobile app (Android & iOS)
QR-based room-level issue reporting
Anonymous complaints for sensitive issues
Performance reports for hostel staff

❓ Why DormFix?

Efficiency → No more verbal complaints or lost registers
Transparency → Students see real-time progress
Accountability → Every issue is tracked & logged
Speed → Faster resolutions, better hostel life

✅ So You Can:

Report hostel issues in seconds
Track complaint status transparently
Reduce admin chaos & manual work
Improve student satisfaction & safety

👥 Team Members 

Naresh Kumar N  – Full Stack & System Design 💻<br>
N S Manish Bhalajy – Backend & Database 🗄<br>
Nevan RG – UI/UX & Student Experience 🎨<br>

🔥 Hostel life shouldn’t be about chasing wardens.
With DormFix, problems get fixed — fast
